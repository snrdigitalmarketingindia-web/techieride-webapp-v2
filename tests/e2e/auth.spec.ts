...
test.describe('🔐 Auth Flow', () => {
  ...
  test('signup page renders', async ({ page }) => {
    ...
    await expect(page.getByRole('heading', { name: /create your techieride account/i })).toBeVisible();
  });

  test('signup redirects /signup to /signup-v2', async ({ page }) => {
    await page.goto('/signup');
    await expect(page).toHaveURL(/\/signup-v2/);
  });

  test('signup shows personal email step first', async ({ page }) => {
    await page.goto('/signup-v2');
    await expect(page.getByText(/personal email/i).first()).toBeVisible();
    await expect(page.getByPlaceholder(/yourname@gmail\.com/i)).toBeVisible();
  });

  test('signup requires valid email to continue', async ({ page }) => {
    await page.goto('/signup-v2');
    await page.getByRole('button', { name: /send verification link/i }).click();
    await expect(page.getByText(/enter.*email|email.*required|valid email/i).first()).toBeVisible({ timeout: 3_000 });
  });
});
