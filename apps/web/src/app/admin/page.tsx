        ...
        <div className="bg-red-50 border border-red-300 rounded-xl p-4">
          ...
          {sosList.map((s) => (
            <div key={s.id} className="mt-2 text-sm text-red-700">
              {s.user?.fullName}{s.user?.trid && <span className="text-xs text-brand-600 font-mono ml-1">({s.user.trid})</span>} — {new Date(s.triggeredAt).toLocaleTimeString('en-IN', { timeZone: 'Asia/Kolkata', hour: '2-digit', minute: '2-digit' })} — {s.lat.toFixed(4)}, {s.lng.toFixed(4)}
            </div>
          ))}
        </div>
      )}
