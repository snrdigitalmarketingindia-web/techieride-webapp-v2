...
const REQUEST_RATE_LIMIT = parseInt(process.env.REQUEST_RATE_LIMIT ?? '5', 10);
const REQUEST_RATE_WINDOW = 3600;
...
let RideRequestsService = RideRequestsService_1 = class RideRequestsService {
    ...
    async create(userId, dto) {
        ...
        if (requestCount === 1)
            await this.redis.expire(rateLimitKey, REQUEST_RATE_WINDOW);
        if (requestCount > REQUEST_RATE_LIMIT) {
            throw new common_1.BadRequestException(`Too many ride requests. You can request up to ${REQUEST_RATE_LIMIT} rides per hour.`);
        }
        ...
        if (ride.archivedAt)
            throw new common_1.BadRequestException('This ride is no longer accepting requests');
    ...
    async approve(requestId, userId) {
        ...
        if (!ride)
            throw new common_1.NotFoundException('Ride not found');
        if (ride.archivedAt)
            throw new common_1.BadRequestException('This ride has been archived and can no longer accept approvals');
    ...
    async cancel(requestId, userId, reason) {
        ...
        if (request.ride?.archivedAt) {
            throw new common_1.BadRequestException('This ride has already passed. No changes can be made to your booking.');
        }
